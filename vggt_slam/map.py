...
class GraphMap:
    ...
    def write_poses_to_file(self, file_name, graph, give_camera_mat=False, kitti_format=False):
        all_poses = self.get_all_cam_matricies(give_camera_mat=give_camera_mat, graph=graph)
        with open(file_name, "w") as f:
            ...
            for submap_index, submap in enumerate(self.ordered_submaps_by_key()):
                ...
                for frame_index, frame_id in enumerate(frame_ids):
                    ...
                    if give_camera_mat:
                        # 4x4 projection matrix — pull the rigid part back out.
                        K, rotation_matrix, t, scale = decompose_camera(pose)
                    else:
                        # Submap.get_all_poses_world already decomposed it for us: the
                        # pose is rigid (R | t), so decomposing again would be wrong.
